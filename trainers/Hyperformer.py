...
class MHSA(nn.Module):
    ...
    def forward(self, x, e):
        ...
        pos_emb = pos_emb.to(torch.float16) ## DEBUG from float32
        #
        k_r = pos_emb.view(V, V, self.num_heads, self.dim // self.num_heads)
        #
        q = q.to(torch.float16) ## DEBUG from float16
        e_k = e_k.to(torch.float16) ## DEBUG from float16
        # self.w1 = self.w1.to(torch.float16)
        # breakpoint()
        b = torch.einsum("bthnc, nmhc->bthnm", q, k_r)
        #
        c = torch.einsum("bthnc, bthmc->bthnm", q, e_k)
        w1 = self.w1.to(torch.float16)
        d = torch.einsum("hc, bthmc->bthm", w1, e_k).unsqueeze(-2) #self.w1
...
class Hyperformer_Model(nn.Module):
    def __init__(self, cfg, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
                 drop_out=0, num_of_heads=9, joint_label=[], **kwargs): #num_point=20
        super(Hyperformer_Model, self).__init__()
        ...
        self.joint_label = [0, 4, 2, 2, 2, 2, 1, 1, 2, 2, 1, 1, 2, 3, 3, 3, 2, 3, 3, 3, 1, 0, 1, 0, 1] #cfg.MODEL.JOINT_LABEL
        ...
        self.fc2 = nn.Linear(24*num_of_heads, 512)
        ...
        nn.init.normal_(self.fc2.weight, 0, math.sqrt(2. / 512))
        bn_init(self.data_bn, 1)
        if drop_out:
