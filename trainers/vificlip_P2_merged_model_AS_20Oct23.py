...
from trainers.Hyperformer import Hyperformer_Model
...
class ViFiCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model, logger, args=None):
        super().__init__()
        print('vificlip',args)
        ...
        ## not load the hyperformer model.
        print('vificlip - before load_model',args)
        self.hyperformer_model = self.load_model(args, cfg)

    def forward(self, image, hyperformer_data=None): #, pose_embedding):
        ...
        # logits = image_features @ text_features.t() # B x C
        # logits = logit_scale * image_features @ text_features.t() # B x C


        ## Introduce skeleton modality
        # pose_features = self.pose_projection(pose_embedding) # B x 512
        
        # get video pose correspondance. VideoPose
        # video_pose_logits = logit_scale * image_features @ pose_features.t() # B x B

        # get text pose correspondance. TextPose
        # text_pose_logits = logit_scale * pose_features @ text_features.t() # B x C

        # (RGB+Pose)_Text
        # image_pose_features = self.rgbpluspose_projection(torch.cat([image_features, pose_embedding], dim=1)) # B x 512
        # rgbpose_text_logits = logit_scale * image_pose_features @ text_features.t()


        # return logits, video_pose_logits, text_pose_logits
        # return logits, text_pose_logits
        # return logits, video_pose_logits
        # return logits, rgbpose_text_logits

        ###### HYPERFORMER ######
        if hyperformer_data is not None:
            pose_embedding = self.hyperformer_model(hyperformer_data)
            return image_features, text_features, pose_embedding
    ...
    def load_model(self, cfg, args=None):
        ...
        Hyp_Model = self.import_class('trainers.Hyperformer.Hyperformer_Model')
...
def returnCLIP(config, logger=None,
               class_names=None, args=None):
    print('returnclip',args)
    ...
    model = ViFiCLIP(config, class_names, clip_model, logger, args)
