...
@PIPELINES.register_module()
class PreprocessPose:
    '''
    Preprocess pose as done in https://github.com/ZhouYuxuanYX/Hyperformer/blob/main/feeders/feeder_ntu.py
    '''

    def __init__(self, split='', p_interval = [], bone=False):
        self.split = split
    ...
    def __call__(self, results):
        ...
        pose_data = results['hformer_data']
        if self.split.lower() not in ['train', 'test']:
            self.split = 'train'

        # for idx in range(len(pose_data)):
            
        valid_frame_num = np.sum(pose_data.sum(0).sum(-1).sum(-1) != 0)

        if self.split.lower() == 'train':
            if len(self.p_interval) == 0:
                self.p_interval = [0.5, 1]
        elif self.split.lower() == 'test':
            self.p_interval = [0.95]

        window_size = 64 # for train and test
        pose_data = valid_crop_resize(pose_data, valid_frame_num, self.p_interval, window_size)

        if self.split == 'train':
            pose_data = random_rot(pose_data)

        if self.bone:
            # print('In bone')
            bone_data_numpy = np.zeros_like(pose_data)
            for v1, v2 in self.ntu_pairs:
                bone_data_numpy[:, :, v1 - 1] = pose_data[:, :, v1 - 1] - pose_data[:, :, v2 - 1]

            # keep spine center's trajectory !!!
            bone_data_numpy[:, :, 20] = pose_data[:, :, 20]
            pose_data = bone_data_numpy    
        else:
            trajectory = pose_data[:, :, 20]
            # let spine of each frame be the joint coordinate center
            pose_data = pose_data - pose_data[:, :, 20:21]
            #
            # ## works well with bone, but has negative effect with joint and distance gate
            pose_data[:, :, 20] = trajectory

        results['hformer_data'] = pose_data
